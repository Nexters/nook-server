...
- 기존 미디어 응답의 `type`, `url`, `sequence`를 유지하고 nullable `thumbnailUrl`을 추가합니다.
...
기존 `SavedPostMediaResponse` 필드는 변경하지 않습니다. 신규 `thumbnailUrl`은 nullable 응답 필드이며,
기존 데이터나 provider가 썸네일을 제공하지 않으면 `null`입니다.
...
기존 클라이언트는 계속 `url`을 사용할 수 있습니다. 신규 클라이언트는 목록에서
`thumbnailUrl ?? url`을 사용하고, 상세에서는 `type`에 따라 `url`을 이미지 또는 영상으로 표시합니다.
