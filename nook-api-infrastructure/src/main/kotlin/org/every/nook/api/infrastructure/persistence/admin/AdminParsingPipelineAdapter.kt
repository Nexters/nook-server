...
class AdminParsingPipelineAdapter(
    ...
    private fun nodes(): List<AdminParsingNode> = listOf(
        ...
        node(
            id = "source-coverage",
            title = "원문 장소 커버리지",
            subtitle = "번호 목록 누락 확인",
            lane = "PLACE",
            x = 1400,
            y = 240,
            summary = "원문의 연속 번호 목록과 추출 단서를 일대일로 대조해 예상 장소 수와 미추출 항목을 기록합니다.",
            inputs = listOf("게시글 본문", "추출 장소 단서"),
            outputs = listOf("예상 장소 수", "미추출 원문 항목"),
            decisions = policyDecisions("source-coverage"),
            sections = policySections("source-coverage"),
        ),
        node(
            id = "text-resolution",
            title = "장소 후보 판정",
