...
type Provider = { provider: string; calls: number; failures: number; units: number; estimatedCostKrw?: number; pricingStatus: string };
type Event = { id: number; provider: string; operation: string; sku: string; status: string; durationMs: number; httpStatus?: number; failureType?: string; estimatedCostKrw?: number; occurredAt: string };
type Summary = { totalCalls: number; failedCalls: number; estimatedCostKrw?: number; unpricedCalls: number; providers: Provider[]; recentEvents: Event[] };
...
export function ExternalProviderUsagePage() {
  ...
  return <Stack spacing={3} sx={{ maxWidth: 1400 }}><Stack direction={{ xs: "column", md: "row" }} sx={{ justifyContent: "space-between", gap: 2 }}><Box><Typography variant="h4">외부 API 사용량</Typography><Typography color="text.secondary">실제 물리 HTTP 요청, 실패율과 공식 단가 기반 예상 비용을 함께 봅니다.</Typography></Box><Stack direction="row" spacing={1}><TextField type="month" size="small" value={month} onChange={(e) => setMonth(e.target.value)} /><TextField select size="small" label="Provider" value={provider} onChange={(e) => setProvider(e.target.value)} sx={{ minWidth: 190 }}><MenuItem value="">전체</MenuItem>{providerOptions.map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField></Stack></Stack>
    ...
    <Stack direction={{ xs: "column", md: "row" }} spacing={2}>{[["총 호출", fmt.format(data.totalCalls)], ["실패", `${fmt.format(data.failedCalls)}건 (${data.totalCalls ? (data.failedCalls / data.totalCalls * 100).toFixed(1) : "0.0"}%)`], ["예상 비용", data.estimatedCostKrw == null ? "미산정" : krw.format(data.estimatedCostKrw)], ["가격 미확정", `${fmt.format(data.unpricedCalls)}건`]].map(([label, value]) => <Card variant="outlined" sx={{ flex: 1 }} key={label}><CardContent><Typography color="text.secondary">{label}</Typography><Typography variant="h5" sx={{ mt: 1 }}>{value}</Typography></CardContent></Card>)}</Stack>
    <UsageTable title="Provider별 집계" headers={["Provider", "호출", "실패", "사용량", "예상 비용", "가격 상태"]}>{data.providers.map((item) => <tr key={item.provider}><td className="mono-text">{item.provider}</td><td>{fmt.format(item.calls)}</td><td>{fmt.format(item.failures)}</td><td>{fmt.format(item.units)}</td><td>{item.estimatedCostKrw == null ? "—" : krw.format(item.estimatedCostKrw)}</td><td><Chip size="small" label={item.pricingStatus} color={item.pricingStatus === "PRICED" ? "success" : "warning"} /></td></tr>)}</UsageTable>
    <UsageTable title="최근 물리 요청" headers={["시각", "Provider", "Operation / SKU", "결과", "지연", "비용"]}>{data.recentEvents.map((event) => <tr key={event.id}><td>{new Date(event.occurredAt).toLocaleString("ko-KR")}</td><td className="mono-text">{event.provider}</td><td><div className="mono-text">{event.operation}</div><small>{event.sku}</small></td><td><Chip size="small" label={event.httpStatus ? `${event.status} · ${event.httpStatus}` : event.status} color={event.status === "SUCCEEDED" ? "success" : "error"} />{event.failureType && <div><small>{event.failureType}</small></div>}</td><td>{fmt.format(event.durationMs)}ms</td><td>{event.estimatedCostKrw == null ? "—" : krw.format(event.estimatedCostKrw)}</td></tr>)}</UsageTable>
  </Stack>;
}
