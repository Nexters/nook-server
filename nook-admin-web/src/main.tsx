...
function PlaceEditDialog({ open, place, onClose }: { open: boolean; place: ManagedPlace; onClose: () => void }) {
  const [name, setName] = useState(place.name); const [address, setAddress] = useState(place.address); const [city, setCity] = useState(place.city ?? ""); const [category, setCategory] = useState(place.category ?? ""); const [phone, setPhone] = useState(place.phoneNumber ?? ""); const [thumbnail, setThumbnail] = useState(place.thumbnailUrl ?? ""); const [photos, setPhotos] = useState(place.photoUrls?.join("\n") ?? ""); const [tags, setTags] = useState(place.representativeTags ?? []); const [hours, setHours] = useState(place.openingHours ? JSON.stringify(place.openingHours, null, 2) : ""); const [reason, setReason] = useState(""); const [saving, setSaving] = useState(false); const notify = useNotify(); const refresh = useRefresh();
  const tagOptions = ["QUIET", "LIVELY", "COZY", "AESTHETIC", "SOLO_DINING", "DATE", "GROUP", "FAMILY", "NEAT", "GENEROUS", "GOOD_VALUE", "UNIQUE", "FRIENDLY", "FAST", "COMFORTABLE"];
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="md"><DialogTitle>공용 장소 정보 수정</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}><Alert severity="warning">게시글 {place.linkedPostCount}개와 저장 사용자 {place.affectedUserCount}명의 화면에 변경된 정보가 표시됩니다.</Alert><Stack direction={{ xs: "column", sm: "row" }} spacing={2}><TextField fullWidth label="장소명" required value={name} onChange={(e) => setName(e.target.value)} /><TextField fullWidth label="도시" value={city} onChange={(e) => setCity(e.target.value)} /></Stack><TextField label="주소" required value={address} onChange={(e) => setAddress(e.target.value)} /><Stack direction={{ xs: "column", sm: "row" }} spacing={2}><TextField fullWidth label="카테고리" value={category} onChange={(e) => setCategory(e.target.value)} /><TextField fullWidth label="전화번호" value={phone} onChange={(e) => setPhone(e.target.value)} /></Stack><TextField label="대표 이미지 URL" value={thumbnail} onChange={(e) => setThumbnail(e.target.value)} /><TextField label="추가 사진 URL" multiline minRows={3} value={photos} onChange={(e) => setPhotos(e.target.value)} helperText="한 줄에 하나씩, 최대 6개" /><Autocomplete multiple options={tagOptions} value={tags} onChange={(_, value) => setTags(value.slice(0, 4))} renderInput={(params) => <TextField {...params} label="대표 태그" helperText="최대 4개" />} /><TextField label="영업시간 JSON" multiline minRows={4} value={hours} onChange={(e) => setHours(e.target.value)} /><TextField label="수정 사유" required multiline minRows={3} value={reason} onChange={(e) => setReason(e.target.value)} /></Stack></DialogContent><DialogActions><Button onClick={onClose}>취소</Button><Button variant="contained" disabled={saving || !name.trim() || !address.trim() || !reason.trim()} onClick={save}>{saving ? <CircularProgress size={20} /> : "수정 저장"}</Button></DialogActions></Dialog>;
}
