...
function PlaceEditDialog({ open, place, onClose }: { open: boolean; place: ManagedPlace; onClose: () => void }) {
  ...
  useEffect(() => { if (!open) return; api<Page<PlaceTagDefinition>>("/place-tags?enabled=true&offset=0&limit=500").then((page) => { setTagOptions(page.items); const activeCodes = new Set(page.items.map((tag) => tag.tagCode)); setTags((current) => current.filter((tag) => activeCodes.has(tag))); }).catch((error) => notify(error instanceof Error ? error.message : "태그 목록을 불러오지 못했습니다.", { type: "error" })); }, [open, notify]);
...
function PlaceTagList() {
  const [tags, setTags] = useState<PlaceTagDefinition[]>([]); const [loading, setLoading] = useState(true); const [createOpen, setCreateOpen] = useState(false); const dragged = useRef<number | null>(null); const timer = useRef<number | undefined>(undefined); const notify = useNotify();
  const load = () => { setLoading(true); api<Page<PlaceTagDefinition>>("/place-tags?offset=0&limit=500").then((page) => setTags(page.items)).catch((error) => notify(error instanceof Error ? error.message : "태그를 불러오지 못했습니다.", { type: "error" })).finally(() => setLoading(false)); };
  useEffect(load, []);
  useEffect(() => () => { if (timer.current) window.clearTimeout(timer.current); }, []);
  const persistOrder = (next: PlaceTagDefinition[]) => { if (timer.current) window.clearTimeout(timer.current); timer.current = window.setTimeout(() => api("/place-tags/order", { method: "PUT", body: JSON.stringify({ tagCodes: next.map((tag) => tag.tagCode), reason: "드래그로 정렬 순서 변경" }) }).then(() => notify("태그 순서를 저장했습니다.", { type: "success" })).catch((error) => { notify(error instanceof Error ? error.message : "순서 저장에 실패했습니다.", { type: "error" }); load(); }), 2000); };
  const moveTo = (target: number) => { if (dragged.current === null || dragged.current === target) return; const next = [...tags]; const [item] = next.splice(dragged.current, 1); next.splice(target, 0, item); dragged.current = target; setTags(next); persistOrder(next); };
  if (loading) return <Box sx={{ p: 4, textAlign: "center" }}><CircularProgress /></Box>;
  return <Box sx={{ p: 3 }}><Stack direction="row" sx={{ mb: 2, justifyContent: "space-between", alignItems: "center" }}><Box><Typography variant="h5">장소 태그</Typography><Typography color="text.secondary">손잡이를 꾹 눌러 이동하세요. 마지막 이동 2초 후 순서를 한 번만 저장합니다.</Typography></Box><Button variant="contained" startIcon={<AddIcon />} onClick={() => setCreateOpen(true)}>태그 추가</Button></Stack><Stack spacing={1}>{tags.map((tag, index) => <Card key={tag.tagCode} variant="outlined" draggable onDragStart={() => { dragged.current = index; }} onDragOver={(event) => { event.preventDefault(); moveTo(index); }} onDragEnd={() => { dragged.current = null; }} sx={{ cursor: "grab" }}><CardContent sx={{ py: 1.5, "&:last-child": { pb: 1.5 } }}><Stack direction="row" spacing={2} sx={{ alignItems: "center" }}><DragIndicatorIcon color="disabled" /><Typography color="text.secondary" sx={{ width: 28 }}>{index + 1}</Typography><Chip size="small" label={categoryLabel(tag.category)} /><Typography sx={{ width: 150, fontWeight: 700 }}>{tag.displayName}</Typography><Stack direction="row" spacing={.5} sx={{ flex: 1, flexWrap: "wrap" }}>{tag.matchingKeywords.slice(0, 5).map((keyword) => <Chip key={keyword} size="small" label={keyword} variant="outlined" />)}</Stack><StatusChip value={tag.enabled ? "사용" : "중지"} /><PlaceTagEditButton tag={tag} onChanged={load} /><PlaceTagDeleteButton tag={tag} options={tags} onChanged={load} /></Stack></CardContent></Card>)}</Stack><PlaceTagCreateDialog open={createOpen} onClose={() => setCreateOpen(false)} onCreated={load} /></Box>;
}

function PlaceTagEditButton({ tag, onChanged }: { tag: PlaceTagDefinition; onChanged: () => void }) {
  const [open, setOpen] = useState(false);
  return <><Button size="small" variant="outlined" onClick={() => setOpen(true)}>수정</Button><PlaceTagEditDialog open={open} tag={tag} onClose={() => setOpen(false)} onChanged={onChanged} /></>;
}

function PlaceTagEditDialog({ open, tag, onClose, onChanged }: { open: boolean; tag: PlaceTagDefinition; onClose: () => void; onChanged: () => void }) {
  const [category, setCategory] = useState(tag.category); const [displayName, setDisplayName] = useState(tag.displayName); const [keywords, setKeywords] = useState(tag.matchingKeywords.join("\n")); const [enabled, setEnabled] = useState(tag.enabled); const [reason, setReason] = useState(""); const [saving, setSaving] = useState(false); const notify = useNotify();
  useEffect(() => { if (!open) return; setCategory(tag.category); setDisplayName(tag.displayName); setKeywords(tag.matchingKeywords.join("\n")); setEnabled(tag.enabled); setReason(""); }, [open, tag]);
  const save = async () => { setSaving(true); try { await api(`/place-tags/${tag.tagCode}`, { method: "PUT", body: JSON.stringify({ category, displayName, matchingKeywords: keywords.split(/\n|,/).map((value) => value.trim()).filter(Boolean), enabled, sortOrder: tag.sortOrder, reason }) }); notify("장소 태그를 수정했습니다.", { type: "success" }); onChanged(); onClose(); } catch (error) { notify(error instanceof Error ? error.message : "태그 수정에 실패했습니다.", { type: "error" }); } finally { setSaving(false); } };
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>장소 태그 수정</DialogTitle><DialogContent><TagFields category={category} setCategory={setCategory} displayName={displayName} setDisplayName={setDisplayName} keywords={keywords} setKeywords={setKeywords} /><FormControlLabel control={<Switch checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />} label={enabled ? "사용 중" : "사용 중지"} /><TextField fullWidth label="수정 사유" required multiline minRows={2} value={reason} onChange={(event) => setReason(event.target.value)} sx={{ mt: 2 }} /></DialogContent><DialogActions><Button onClick={onClose}>취소</Button><Button variant="contained" disabled={saving || !displayName.trim() || !keywords.trim() || !reason.trim()} onClick={save}>{saving ? <CircularProgress size={20} /> : "수정 저장"}</Button></DialogActions></Dialog>;
}

function TagFields({ category, setCategory, displayName, setDisplayName, keywords, setKeywords }: { category: string; setCategory: (value: string) => void; displayName: string; setDisplayName: (value: string) => void; keywords: string; setKeywords: (value: string) => void }) { return <Stack spacing={2} sx={{ my: 1 }}><TextField select label="분류" value={category} onChange={(event) => setCategory(event.target.value)}>{placeTagCategoryChoices.map((choice) => <MenuItem key={choice.id} value={choice.id}>{choice.name}</MenuItem>)}</TextField><TextField label="노출 태그명" required value={displayName} onChange={(event) => setDisplayName(event.target.value)} /><TextField label="본문 매칭 키워드" required multiline minRows={6} value={keywords} onChange={(event) => setKeywords(event.target.value)} helperText="줄바꿈 또는 쉼표로 구분하며 최대 20개입니다." /></Stack>; }

function PlaceTagCreateDialog({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) { const [category, setCategory] = useState("ATMOSPHERE"); const [displayName, setDisplayName] = useState(""); const [keywords, setKeywords] = useState(""); const [reason, setReason] = useState(""); const [saving, setSaving] = useState(false); const notify = useNotify(); const save = async () => { setSaving(true); try { await api("/place-tags", { method: "POST", body: JSON.stringify({ category, displayName, matchingKeywords: keywords.split(/\n|,/).map((value) => value.trim()).filter(Boolean), reason }) }); notify("장소 태그를 추가했습니다.", { type: "success" }); setDisplayName(""); setKeywords(""); setReason(""); onCreated(); onClose(); } catch (error) { notify(error instanceof Error ? error.message : "태그 추가에 실패했습니다.", { type: "error" }); } finally { setSaving(false); } }; return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>장소 태그 추가</DialogTitle><DialogContent><TagFields category={category} setCategory={setCategory} displayName={displayName} setDisplayName={setDisplayName} keywords={keywords} setKeywords={setKeywords} /><TextField fullWidth label="추가 사유" required multiline minRows={2} value={reason} onChange={(event) => setReason(event.target.value)} /></DialogContent><DialogActions><Button onClick={onClose}>취소</Button><Button variant="contained" disabled={saving || !displayName.trim() || !keywords.trim() || !reason.trim()} onClick={save}>추가</Button></DialogActions></Dialog>; }

function PlaceTagDeleteButton({ tag, options, onChanged }: { tag: PlaceTagDefinition; options: PlaceTagDefinition[]; onChanged: () => void }) { const [open, setOpen] = useState(false); const replacements = options.filter((candidate) => candidate.enabled && candidate.tagCode !== tag.tagCode); const [replacement, setReplacement] = useState(""); const [reason, setReason] = useState(""); const notify = useNotify(); const remove = async () => { try { await api(`/place-tags/${tag.tagCode}`, { method: "DELETE", body: JSON.stringify({ replacementTagCode: replacement, reason }) }); notify("기존 장소를 대체 태그로 이관하고 태그를 삭제했습니다.", { type: "success" }); onChanged(); setOpen(false); } catch (error) { notify(error instanceof Error ? error.message : "태그 삭제에 실패했습니다.", { type: "error" }); } }; return <><Button size="small" color="error" startIcon={<DeleteIcon />} onClick={() => setOpen(true)}>삭제</Button><Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm"><DialogTitle>장소 태그 삭제</DialogTitle><DialogContent><Alert severity="warning" sx={{ mb: 2 }}>‘{tag.displayName}’이 붙은 모든 장소와 근거 데이터를 아래 태그로 이관합니다.</Alert><TextField select fullWidth label="대체 태그" required value={replacement} onChange={(event) => setReplacement(event.target.value)}>{replacements.map((candidate) => <MenuItem key={candidate.tagCode} value={candidate.tagCode}>{candidate.displayName}</MenuItem>)}</TextField><TextField fullWidth label="삭제 사유" required multiline minRows={2} value={reason} onChange={(event) => setReason(event.target.value)} sx={{ mt: 2 }} /></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>취소</Button><Button color="error" variant="contained" disabled={!replacement || !reason.trim()} onClick={remove}>이관 후 삭제</Button></DialogActions></Dialog></>; }
