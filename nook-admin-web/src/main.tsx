...
function Dashboard() {
  ...
  return <Stack spacing={3} sx={{ maxWidth: 960 }}><Box><Typography variant="overline" color="primary.main" sx={{ fontFamily: '"Roboto Mono", monospace', fontWeight: 600, letterSpacing: ".08em" }}>NOOK OPERATIONS</Typography><Typography variant="h4">운영 콘솔</Typography><Typography color="text.secondary" sx={{ mt: .5 }}>게시글과 장소 데이터를 안전하게 검수하고 관리하세요.</Typography></Box><Alert severity="success" variant="outlined">{email ? <><Box component="span" className="mono-text">{email}</Box> 계정으로 인증되었습니다.</> : "인증 정보를 확인하고 있습니다."}</Alert><Stack direction={{ xs: "column", md: "row" }} spacing={2}><Card variant="outlined" sx={{ flex: 1 }}><CardContent><Typography variant="overline" color="text.secondary">DATA POLICY</Typography><Typography variant="h6" sx={{ mt: .5 }}>기존 사용자 데이터 보호</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>공용 매핑 교정은 이후 저장 건에 적용되며 기존 사용자의 저장 장소는 변경하지 않습니다.</Typography></CardContent></Card><Card variant="outlined" sx={{ flex: 1 }}><CardContent><Typography variant="overline" color="text.secondary">AUDIT POLICY</Typography><Typography variant="h6" sx={{ mt: .5 }}>모든 변경 기록</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>운영자와 수정 사유, 변경 전후 값이 감사 로그에 기록됩니다.</Typography></CardContent></Card></Stack></Stack>;
}
...
function PostPanel() {
  ...
  return <Stack spacing={3}><Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: "wrap" }}><StatusChip value={`게시글 ${record.contentParsingStatus}`} /><StatusChip value={`장소 ${record.placeParsingStatus ?? "미시작"}`} /><StatusChip value={record.mappingReviewed ? "완료" : "미검수"} /></Stack>{failure && <Alert severity="error">{failure}</Alert>}<Card variant="outlined"><CardContent><Typography variant="h6">{record.title ?? "제목 없음"}</Typography><Typography color="text.secondary" className="mono-text">{record.authorIdentifier ?? "작성자 미상"} · 저장 사용자 {record.savedUserCount}명</Typography><Typography sx={{ whiteSpace: "pre-wrap", mt: 2, color: "#353C46" }}>{record.body}</Typography></CardContent></Card><Box><Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ justifyContent: "space-between", alignItems: { sm: "center" } }}><Box><Typography variant="h6">공용 장소 매핑</Typography><Typography color="text.secondary" variant="body2">게시글에 연결된 장소 {record.places.length}개</Typography></Box><Button variant="contained" onClick={() => setOpen(true)}>매핑 교정</Button></Stack><Stack spacing={1.5} sx={{ mt: 2 }}>{record.places.map((place) => <Card variant="outlined" key={place.id}><CardContent><Stack direction="row" spacing={1.5} sx={{ alignItems: "flex-start" }}><Box className="summary-icon">{place.sequence + 1}</Box><Box><Typography variant="subtitle1">{place.name}</Typography><Typography color="text.secondary">{place.address}</Typography><Typography variant="caption" className="mono-text" color="text.disabled">{place.provider} · {place.externalPlaceId}</Typography></Box></Stack></CardContent></Card>)}</Stack></Box><MappingDialog open={open} post={record} onClose={() => setOpen(false)} /></Stack>;
}
...
function PlacePanel() {
  ...
  return <Stack spacing={3}><Alert severity="warning" variant="outlined">이 장소는 게시글 {record.linkedPostCount}개와 저장 사용자 {record.affectedUserCount}명에게 노출됩니다. 장소명·주소 수정은 기존 사용자 화면에도 즉시 반영됩니다.</Alert><Card variant="outlined"><CardContent><Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ justifyContent: "space-between", alignItems: { sm: "center" } }}><Box><Stack direction="row" spacing={1} sx={{ alignItems: "center" }}><Typography variant="h5">{record.name}</Typography><Chip size="small" variant="outlined" label={record.provider} /></Stack><Typography color="text.secondary" sx={{ mt: .75 }}>{record.address}</Typography><Typography variant="caption" className="mono-text" color="text.disabled">{record.externalPlaceId}</Typography></Box><Button variant="contained" onClick={() => setOpen(true)}>장소 정보 수정</Button></Stack></CardContent></Card><Box><Typography variant="h6">연결 게시글</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>이 장소를 참조하는 게시글 {record.posts.length}개</Typography><Stack spacing={1.5}>{record.posts.map((post) => <Card variant="outlined" key={post.id}><CardContent><Typography variant="subtitle1">{post.title ?? "제목 없음"}</Typography><Typography color="text.secondary" className="mono-text">{post.authorIdentifier ?? "작성자 미상"}</Typography><Stack direction="row" spacing={1} sx={{ mt: 1 }}><Button size="small" href={`#/posts/${post.id}/show`}>게시글 관리에서 보기</Button><Button size="small" href={post.canonicalUrl} target="_blank" endIcon={<OpenInNewIcon />}>원문</Button></Stack></CardContent></Card>)}</Stack></Box><PlaceEditDialog open={open} place={record} onClose={() => setOpen(false)} /></Stack>;
}
